    ...
    public function index(Request $request): View
    {
        ...
        $nationalRankings = Cache::remember('national_rankings_' . $today, 3600, function () use ($today) {
            ...
            $summary = SneezeLog::whereDate('sneeze_logs.created_at', $today)
                ...
                ->select(
                    'users.prefecture',
                    DB::raw('SUM(sneeze_logs.count) as total'),
                    DB::raw('AVG(sneeze_logs.level) as avg')
                )
        ...
        $personalRankings = SneezeLog::whereDate('sneeze_logs.created_at', $today)
        ...
        if (Auth::check()) {
            ...
            $myStats = SneezeLog::where('user_id', $user->id)
                ->whereDate('sneeze_logs.created_at', $today)
            ...
            if ($myStats && $myStats->total_count > 0) {
                ...
                $rank = SneezeLog::whereDate('sneeze_logs.created_at', $today)
                    ->groupBy('user_id')
